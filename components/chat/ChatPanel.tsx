...
export const ChatPanel = () => {
    ...
    <div className="flex flex-col h-full bg-card/40 backdrop-blur-2xl">
      ...
      {isMobile && (
        <div 
          ...
          className="w-12 h-1 bg-white/20 rounded-full mx-auto my-3 shrink-0 cursor-pointer hover:bg-white/35 transition-colors"
          title="Swipe or tap to close chat"
        />
      )}
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-border bg-card/10 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className={`p-1.5 rounded-lg transition-colors cursor-pointer ${showUpload ? "bg-accent text-white" : "hover:bg-card border border-border text-foreground/50"}`}
        ...
        {isMobile && (
          <button
            onClick={() => setChatOpen(false)}
            className="p-1.5 rounded-lg border border-border hover:border-destructive/30 text-foreground/50 hover:text-destructive transition-colors cursor-pointer outline-none"
      ...
      >
        <AnimatePresence mode="wait">
          {messages.length === 0 && !showUpload && (
            <motion.div 
              ...
              className="h-full flex flex-col items-center justify-center text-center p-4 sm:p-8"
            >
              ...
              <div className="flex flex-col gap-3 w-full max-w-xs">
                {[
                  ...
                  <button
                    ...
                    className="flex items-center gap-3 p-3 text-left text-xs bg-card border border-border rounded-xl hover:border-accent/50 hover:bg-accent/5 transition-all text-foreground/70 font-sans group cursor-pointer"
                  >
          ...
          {showUpload && (
            ...
            >
              <div className="flex items-center justify-between mb-3 px-2">
                ...
                <button onClick={() => setShowUpload(false)} className="text-foreground/40 hover:text-foreground transition-colors cursor-pointer">
                  <X size={16} />
                </button>
              </div>
        ...
        {loading && (
          ...
          >
            <div className="w-8 h-8 rounded-lg bg-card border border-border text-accent flex items-center justify-center shrink-0">
      ...
      <div className="p-4 border-t border-border bg-panel flex-shrink-0">
        {messages.length === 0 && !showUpload && (
          <motion.button 
            ...
            className="w-full mb-3 py-3 bg-accent text-white rounded-xl font-bold text-sm hover:opacity-90 transition-all flex items-center justify-center gap-2 shadow-lg shadow-accent/20 cursor-pointer border border-accent/20"
          >
            <CloudUpload size={18} />
        ...
        <form onSubmit={handleSubmit} className="relative flex items-center gap-2">
          <div className="relative flex-1 group">
            <button
              type="button"
              onClick={() => setShowUpload(!showUpload)}
              className={`absolute left-3 top-1/2 -translate-y-1/2 p-1.5 rounded-lg transition-all cursor-pointer ${showUpload ? "bg-accent text-white" : "text-foreground/40 hover:text-foreground hover:bg-card"}`}
              title="Upload File"
            >
              <Paperclip size={18} />
            </button>
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-accent text-white rounded-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-all cursor-pointer"
            >
              <Send size={18} />
            </button>
          </div>
  ...
  return (
    <AnimatePresence>
      {isChatOpen && (
        <>
          {/* Backdrop blur overlay for Mobile Bottom Sheets */}
          {isMobile && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
